...
const ListaCanciones = ({ canciones }) => {
  ...
  useEffect(() => {
    setCancionesFiltradas(
      canciones.filter(
        (cancion) =>
          cancion.songTitle.toLowerCase().includes(input.toLowerCase()) ||
          cancion.artist.toLowerCase().includes(input.toLowerCase()) ||
          cancion.genre.toLowerCase().includes(input.toLowerCase())
      )
    );
  }, [input, canciones]);
