...
const getPlaylistID = async (req, res) => {
    try {
        const playlistDB = await Playlist.findOne({ _id: req.params.id });
        return res.status(200).json(playlistDB);
    } catch (error) {
        console.log(error);
        return res.status(500).json({
...
const putPlaylist = async (req, res) => {
    ...
    try {
        const playListActualizada = await Playlist.findByIdAndUpdate(id, cuerpo, opciones);

        if (!playListActualizada) {
            return res.status(400).json({ error: 'No existe la playlist' });
        }

        return res.status(200).json({ mensaje: 'Playlist actualizada', cuerpo: playListActualizada });
